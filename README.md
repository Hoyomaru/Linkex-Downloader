...
一部ファイルだけ欲しい場合は **ファイルを選ぶ** を押して対象を選択します。
...
診断、署名テスト、保存先変更、容量skip再試行、安全破棄、互換方式、安全方式の説明など低頻度操作は **詳細** にまとめています。
...
| **詳細** | 保存先変更、再解析、署名テスト、診断ログ保存、状態再表示、容量skip再試行、安全破棄、互換方式、安全方式の説明、折りたたみログ表示を格納 |
| **− / +** | パネルを最小化/展開 |

通常の高速方式で問題がある場合は **詳細 → 互換モード** から、従来の「DL → VERIFY → LOCAL_COMMITTED → DELETE」方式を全件または選択ファイルに対して使用できます。通常画面には互換ボタンを常時表示しません。
...
保存先HandleもIndexedDBへ保存します。複数ファイルQueueではDirectoryHandle、単一ファイル直接保存ではFileHandleを保存します。ただしブラウザ再起動等でpermission再確認が必要になる場合があります。
