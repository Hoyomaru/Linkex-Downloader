...
""",
    """        onStatus?.(`DOWNLOADING [${index+1}/${job.items.length}]\\n${item.source.remotePath}\\n再開可能 / attempt ${attempt}`);
        ...
        return tx;
""",
    "download phase end and UI rate",
)
...
test_path = Path("tests/performance-telemetry.test.js")
...
marker = "The exact persisted schema may change during implementation, but it must remain operation-scoped so later multi-worker work cannot collide through a single global state entry.\n"
...
perf = replace_once(perf, marker, note, "performance implementation note")
