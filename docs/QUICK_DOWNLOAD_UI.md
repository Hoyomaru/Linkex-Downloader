...
詳細ログは初期画面に常時表示せず、通常時は進捗バー、完了件数、合計転送速度、概算ETA、現在ダウンロード中のファイル、1行の状態表示を見せる。速度は実行中workerのfresh telemetryだけを合計し、ETAは平滑化した速度を使う。開始直後やtelemetryが古い場合は推測値を出さず「計測中」とする。処理履歴自体は従来どおり保持し、診断ログJSON出力にも含める。
